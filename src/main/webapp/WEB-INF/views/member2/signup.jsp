...
document.querySelector("#userIdInput1").addEventListener("keyup", function() {
 	availableId = false;
 	document.querySelector("#userIdText1").innerText="아이디 중복확인을 다시 해주세요"
	enableSubmitButton();
});
...
document.querySelector("#userIdExistButton1").addEventListener("click", function() {
	...
	fetch(ctx + "/member/existId/" + userId)
		...
		.then(data => {
			...
			if (data.status == "not exist") {
				availableId = true;
				enableSubmitButton();// 실행흐름이 브라우저의 이벤트에 따라서 달라지니까,  이벤트가 끝날때마다 실행해줘야하는 함수를 이벤트마다 붙여줌
			}
		}); 
...
function matchPassword() {
	if (document.querySelector("#passwordInput1").value == document.querySelector("#passwordInput2").value) {
		document.querySelector("#passwordText1").innerText = "패스워드가 일치합니다.";
		availablePassword = true;
	} else {
		document.querySelector("#passwordText1").innerText = "패스워드가 일치하지 않습니다.";
	}	
	enableSubmitButton();
}
...
function enableSubmitButton() {
	if (availableId && availableEmail && availablePassword) {
		document.querySelector("#submitButton1").removeAttribute("disabled")
	} 
}
